...
def run_original_paper_point(utils, params_env_base: dict, sims: int, seed: int) -> Tuple[PaperPoint, object]:
    """
    Run one parameter point using the original implementation.

    This mirrors the notebook pattern:
        mm_aux = utils.mm_with_fads(**params_aux)
        np.random.seed(seed)
        PnL_val = mm_aux.PnL(100_000)
    """
...
def simulate_arrivals_filter_from_mm(mm_aux, sims: int, Nt: int, J: int, seed: int, verbose: bool = True) -> Tuple[float, float]:
    ...
    for it in iterator:
        ...
        # Belief update: Markov prediction followed by Poisson likelihood update.
...
def run_q(mode: str, sims_paper: int, sims_arr: int, Nt_arr: int, J: int, seed: int, outdir: str):
    ...
    plot_sweep(TABLE1_Q_VALUES, series, r"$q$", r"$q$ sweep: original FI/CJP/PI" + (" + ARR" if "arrivals" in mode else ""), os.path.join(outdir, "q_sweep.png"))

    # Also save paper Table 1 reference for quick visual check.
    ref_series = dict(TABLE1_Q_MEANS)
    plot_sweep(TABLE1_Q_VALUES, ref_series, r"$q$", r"Table 1 reference: $q$ sweep", os.path.join(outdir, "q_sweep_table1_reference.png"))
...
def run_gamma(mode: str, sims_paper: int, sims_arr: int, Nt_arr: int, J: int, seed: int, outdir: str):
    ...
    plot_sweep(TABLE1_GAMMA_VALUES, series, r"$\gamma$", r"$\gamma$ sweep: original FI/CJP/PI" + (" + ARR" if "arrivals" in mode else ""), os.path.join(outdir, "gamma_sweep.png"))

    ref_series = dict(TABLE1_GAMMA_MEANS)
    plot_sweep(TABLE1_GAMMA_VALUES, ref_series, r"$\gamma$", r"Table 1 reference: $\gamma$ sweep", os.path.join(outdir, "gamma_sweep_table1_reference.png"))
