...
def main():
    ...
    parser.add_argument("--q_values", type=str, default="0.40,0.45,0.50,0.55,0.60")
    parser.add_argument("--gamma_values", type=str, default="1.50,2.00,2.50,3.00")
    ...
    parser.add_argument("--seed", type=int, default=575757)
    parser.add_argument("--alpha", type=float, default=0.05)
    parser.add_argument("--outdir", type=str,
                        default="outputs_clean/57_j3_2d_common_fiplug_benchmark")
    ...
    pbar = tqdm(total=len(q_values) * len(gamma_values), desc="J=3 2D common FI plug-in benchmark")
    ...
    if not args.no_plots:
        plot_heatmap(
            q_values, gamma_values, diff,
            os.path.join(args.outdir, "arr_minus_pi_fiplug_heatmap.png"),
            rf"J=3 common FI plug-in: $\bar J^{{ARR}}-\bar J^{{PI}}$ ({suffix})",
            r"$\bar J^{ARR}-\bar J^{PI}$",
        )
        plot_heatmap(
            q_values, gamma_values, diff_sig,
            os.path.join(args.outdir, "arr_minus_pi_fiplug_sig_heatmap.png"),
            rf"Significance-filtered J=3 common FI plug-in ({suffix})",
            r"displayed $\bar J^{ARR}-\bar J^{PI}$",
        )
        plot_heatmap(
            q_values, gamma_values, psi_mat,
    ...
    summary = vars(args)
    summary.update({
        "purpose": "J=3 2D common FI plug-in benchmark for ARR-NN averaged sweep.",
    ...
    with open(os.path.join(args.outdir, "j3_2d_common_fiplug_benchmark_summary.json"), "w") as f:
